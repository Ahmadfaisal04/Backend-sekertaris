...
func (c *PermohonanSuratController) UpdateStatus(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	// Validasi status
	if requestBody.Status != model.Pending && requestBody.Status != model.Diproses && requestBody.Status != model.Selesai {
			http.Error(w, `{"error": "Status must be 'Pending', 'Diproses', or 'Selesai'"}`, http.StatusBadRequest)
			return
	}

	// Perbarui status menggunakan service
	err = c.service.UpdateStatusByID(id, requestBody.Status)
	if err != nil {
			log.Println("Error updating status:", err)
			if strings.Contains(err.Error(), "tidak ada permohonan surat") {
					http.Error(w, `{"error": "Permohonan surat not found"}`, http.StatusNotFound)
					return
			}
